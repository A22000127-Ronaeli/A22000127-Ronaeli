const menus = [
  {
    name: "Affogato_Coffee",
    label_name: "Affogato Coffee",
    harga: 34000,
  },
  {
    name: "Expresso_Coffee",
    label_name: "Expresso Coffee",
    harga: 20000,
  },
  {
    name: "Flat_Coffee",
    label_name: "Flat White Coffee",
    harga: 56000,
  },
  {
    name: "Latte_Coffee",
    label_name: "Latte Coffee",
    harga: 40000,
  },
];

let [dataBarang, listBarang, alert, elGrandTotal, grandTotal] = [[], document.getElementById("list-barang"), document.getElementById("alert"), document.getElementById("grandtotal"), 0];
...
const addBarang = () => {
  ...
  if (!uniqBarang().includes(daftarMenu.name) && daftarMenu.name) {
    dataBarang.push([daftarMenu.label_name, daftarMenu.name, daftarMenu.harga, jumlah, total]);
    showBarang();
    alert.innerHTML = "";
    return;
  } else if (uniqBarang().includes(daftarMenu.name)) {
    alert.innerHTML = "Coffee Sudah Ada!";
    return;
  }
...
const editMenu = (id, opsi = null) => {
  ...
  if (opsi == "min") {
    inputNum.value = Number(inputNum.value) == 1 ? 1 : Number(inputNum.value) - 1;
  }
  let total = daftarMenu.harga * Number(inputNum.value);
  dataBarang[id] = [dataBarang[id][0], dataBarang[id][1], dataBarang[id][2], Number(inputNum.value), total];
